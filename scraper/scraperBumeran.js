...
const busquedaBumeran = async (search, location = '') => {
    ...
    while (stop < 1) {
        ...
        for (const productHandle of productHandles) {
            ...
            try {
                name = await page.evaluate(el => el.querySelector("h2").textContent, productHandle);
            } catch (error) { }
            ...
            try {
                location = await page.evaluate(el => el.querySelector("a > div > div:nth-child(2) > div > div:nth-child(1) > h3").textContent, productHandle);
            } catch (error) { }
